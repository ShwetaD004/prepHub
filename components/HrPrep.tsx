...
import { generateHrQuestion, generateHrFollowUpQuestion, getHrInterviewFeedback, getHrFinalReport } from '../services/geminiService';
import { saveInterviewHistory } from '../services/firestoreService';
import { Feature, InterviewHistory, HrDataReference } from '../types';
...
const HrPrep: React.FC<HrPrepProps> = ({ setIsQuizActive, user, onNavigate }) => {
    ...
    const handleSubmitAnswer = async () => {
        ...
        try {
            ...
            const followUpQuestion = await generateHrFollowUpQuestion(updatedConversation);
    ...
    const handleEndSession = async () => {
        ...
        try {
            ...
            const perQuestionFeedback = await getHrInterviewFeedback(finalConversation);
            
            setLoadingMessage('Synthesizing your final report...');
            const { rating, summary, report } = await getHrFinalReport(finalConversation, perQuestionFeedback);
        ...
        return (
            <div className="p-4 md:p-8 max-w-2xl mx-auto animate-fade-in flex items-center justify-center min-h-[80vh]">
                <Card className="text-center">
                    <h2 className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-primary to-secondary">HR Interview Simulation</h2>
                    <p className="text-on-secondary mt-4 mb-8">Master behavioral questions in a realistic, voice-enabled interview. Get a comprehensive report at the end.</p>
                    <button 
                        onClick={startInterview}
                        disabled={isLoading}
                        className="w-full bg-gradient-to-r from-primary to-secondary text-white font-bold py-3 px-12 rounded-lg text-lg hover:shadow-xl transform hover:scale-105 transition disabled:from-slate-400"
                    >
             ...
             <div className="p-4 md:p-8 max-w-4xl mx-auto animate-fade-in">
                <Card>
                    ...
                    <div className="mt-8 flex gap-4 justify-center">
                        ...
                        <button onClick={() => setFlow('setup')} className="bg-gradient-to-r from-primary to-secondary text-white font-bold py-3 px-8 rounded-lg hover:shadow-lg transition">New Interview</button>
                    </div>
                </Card>
            </div>
        );
        ...
        <div className="p-2 md:p-4 h-screen bg-transparent">
            ...
            <div className="bg-surface rounded-2xl shadow-2xl flex flex-col h-full max-w-4xl mx-auto border border-white/20">
                <header className="flex justify-between items-center p-4 border-b border-slate-200 flex-shrink-0">
                  <div>
                    <h2 className="text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-primary to-secondary">Behavioral Interview</h2>
                    ...
                    {conversation.map((turn, index) => (
                        <React.Fragment key={index}>
                            ...
                            <div className="flex justify-end animate-fade-in">
                                <div className="bg-gradient-to-br from-primary to-secondary text-white rounded-xl rounded-br-none p-4 max-w-lg shadow-md">
                ...
                <footer className="border-t border-slate-200 p-4 bg-white/70 backdrop-blur-sm flex-shrink-0">
                    ...
                    <div className="mt-4 flex flex-col sm:flex-row gap-3 justify-end">
                        ...
                        <button onClick={handleSubmitAnswer} disabled={!currentAnswer.trim() || isLoading || !currentQuestion}
                            className="bg-primary text-white font-bold py-2.5 px-6 rounded-lg hover:bg-primary-dark shadow-sm hover:shadow-md transition disabled:bg-slate-400 disabled:shadow-none">
                            Submit Answer
                        </button>
                    </div>
                </footer>
